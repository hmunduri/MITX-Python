...
# To run these tests, simply run this file (open up in your IDE, then run the
# file as normal)
    ...
    for (word, n) in words.keys():
        ...
        if score != words[(word, n)]:
            ...
            print('\tExpected', words[(word, n)],
                  "points but got '" + str(score) + "' for word '" + word +\
                  "', n=" + str(n))
            failure = True
    if not failure:
...
def test_is_valid_word(word_list):
    ...
    if not is_valid_word(word, copied_hand, word_list):
        ...
        if copied_hand != original_hand:
            ...
            print('\tAt this point, hand ought to be', original_hand,
                  'but it is', copied_hand)
